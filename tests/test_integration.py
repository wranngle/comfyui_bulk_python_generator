"""End-to-end integration tests against real media on D:\\.

Marked `integration` so they only run via `pytest -m integration`.

NOTE: these tests must use a /mnt/c-rooted scratch dir, not /tmp, so Windows
ffmpeg.exe can read/write the staged files via WSL interop.
"""
...
@pytest.fixture
def win_tmp_path():
    """A scratch dir under /mnt/c so Windows ffmpeg can see it."""
    base = Path("/mnt/c/Users/root/dev/comfyui_bulk_python_generator/.testtmp")
...
def test_extract_from_real_clip(win_tmp_path, real_test_clip):
    ...
    # Real clip may not have valid ComfyUI metadata — just assert no exceptions and a known result.
...
def test_pipeline_single_no_effects(win_tmp_path, real_test_clip):
    ...
    shutil.copyfile(to_posix(real_test_clip), str(staged))  # copyfile skips permissions (WSL/NTFS)
    ...
    cfg.paths.favorites_root = str(win_tmp_path / "no_organize")  # disable organize side-effect
