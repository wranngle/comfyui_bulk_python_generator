...
@pytest.mark.parametrize("ratio,expected_count", [
    (1.0, 1),
    (0.7, 1),
    (1.5, 1),
    (3.0, 2),       # decomposes via sqrt
    (0.25, 2),
    (10.0, 2),
])
def test_atempo_chain_count(ratio, expected_count):
    chain = atempo_chain(ratio)
    assert chain.count("atempo=") == expected_count
